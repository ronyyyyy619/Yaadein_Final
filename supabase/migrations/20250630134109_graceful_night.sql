...
DO $$ 
BEGIN
  -- Check if "Users can view families they created" policy exists
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Users can view families they created"
      ON families
      FOR SELECT
      TO authenticated
  ...
  -- Check if "Users can view families they belong to" policy exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'families' 
    AND schemaname = 'public' 
    ...
    -- Create policy only if it doesn't exist
    EXECUTE 'CREATE POLICY "Users can view families they belong to"
      ON families
      FOR SELECT
      TO authenticated
      USING (
